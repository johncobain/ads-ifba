...
public record UserFormDto (
    ...
    @Schema(description = "Username", example = "John Dee")
    String name,
    @NotBlank(message = "Login can not be blank")
    @Schema(description = "User Login", example = "john@dee.com")
    @Size(min = 6, max = 30, message = "Login must be between 6 and 30 characters")
    String login,
    @NotBlank(message = "Password can not be blank")
    @Size(min = 6, message = "Password must be at least 6 characters")
    @Schema(description = "User Password", example = "123superSecurePass")
    String password
    ){
